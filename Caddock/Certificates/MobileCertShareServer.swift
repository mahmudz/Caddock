...
final class MobileCertShareServer {
    ...
    private let queue = DispatchQueue(label: "dev.mahmudz.Caddock.mobile-cert")
                ...
                case .ready:
                    ...
                    if let port = listener.port?.rawValue,
                       let ip = Self.localIPv4Address() {
                        self?.downloadURL = URL(string: "http://\(ip):\(port)/Caddock-Root-CA.crt")
                    } else if let port = listener.port?.rawValue {
                        self?.downloadURL = URL(string: "http://127.0.0.1:\(port)/Caddock-Root-CA.crt")
                    }
    ...
    private func handle(connection: NWConnection) {
        ...
        connection.receive(minimumIncompleteLength: 1, maximumLength: 64_000) { [weak self] data, _, _, _ in
            ...
            Connection: close\r
            Content-Disposition: attachment; filename="Caddock-Root-CA.crt"\r
            \r
