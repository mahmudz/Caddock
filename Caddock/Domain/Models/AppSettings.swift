...
final class AppSettings {
    private enum Keys {
        ...
        static let legacyHasCompletedCaddyOnboarding = "hasCompletedCaddyOnboarding"
    }
    ...
    /// Cache only — never the sole source of truth for CA trust.
    ...
    init(defaults: UserDefaults = .standard) {
        ...
        if defaults.object(forKey: Keys.hasCompletedSetup) != nil {
            self.hasCompletedSetup = defaults.bool(forKey: Keys.hasCompletedSetup)
        } else {
            self.hasCompletedSetup = defaults.bool(forKey: Keys.legacyHasCompletedCaddyOnboarding)
        }
    }
}
