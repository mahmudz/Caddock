...
struct CertificateSettingsPane: View {
    ...
    var body: some View {
        Form {
            ...
            Section {
                ...
                LabeledContent {
                    Button("Open Keychain Access", action: openKeychain)
                        .controlSize(.small)
                } label: {
            ...
            if trustStatus == .installedNotTrusted {
                Section {
                    VStack(alignment: .leading, spacing: 6) {
                        Text("The current Root CA file is present but not trusted in Keychain. Re-install, or finish trust manually:")
                        Text("1. Click Open Keychain Access above")
                        Text("2. Select login (or System) keychain in the sidebar")
                        Text("3. Find the certificate matching this Caddy Root CA")
    ...
    private var statusFooter: String {
        ...
        case .installedNotTrusted:
            return "Certificate exists but is not trusted. Re-install the Root CA, or set Always Trust in Keychain Access."
        case .notInstalled:
    ...
    private func openKeychain() {
        do {
            try KeychainAccessOpener.open()
            actionError = nil
        } catch {
            actionError = error.localizedDescription
        }
