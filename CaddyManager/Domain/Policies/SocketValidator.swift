...
enum SocketValidator {
    /// True if something accepts connections at `target`.
    /// Unix paths (`/tmp/php-fpm.sock`, `unix//tmp/php-fpm.sock`) or TCP (`127.0.0.1:9000`).
    /// Uses a real `connect()` probe — not `FileManager.fileExists`.
