...
enum VhostValidator {
    ...
    private static let proxyHostPortRegex = try! NSRegularExpression(
    ...
    static func validate(
        _ vhost: Vhost,
        existing: [Vhost],
        socketProbe: (String) -> Bool = SocketValidator.isListening
    ) -> [VhostValidationIssue] {
        ...
        case .phpSite:
            ...
            } else if let socketPath = vhost.phpSocketPath, !socketProbe(socketPath) {
        ...
        case .reverseProxy:
            ...
            } else if let target = vhost.proxyTarget, !isValidProxyTarget(target) {
                issues.append(.init(severity: .error, message: "Proxy target must look like host:port (e.g. 127.0.0.1:3000)."))
            }
    ...
    static func isValid(_ vhost: Vhost, existing: [Vhost], socketProbe: (String) -> Bool = SocketValidator.isListening) -> Bool {
        !validate(vhost, existing: existing, socketProbe: socketProbe).contains { $0.severity == .error }
    }

    static func isValidProxyTarget(_ value: String) -> Bool {
        ...
        guard proxyHostPortRegex.firstMatch(in: candidate, range: range) != nil else { return false }

        if let colon = candidate.lastIndex(of: ":") {
            let portString = String(candidate[candidate.index(after: colon)...])
