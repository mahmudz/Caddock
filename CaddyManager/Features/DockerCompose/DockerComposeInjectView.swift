...
struct DockerComposeInjectView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Inject CaddyManager's Root CA and domain extra_hosts into a docker-compose.yml override.")
            ...
            GroupBox("After inject") {
                VStack(alignment: .leading, spacing: 4) {
                    Text("Creates docker-compose.override.yml and .caddymanager-root-ca.crt")
                    Text("Debian/Ubuntu: update-ca-certificates")
                    Text("Node: NODE_EXTRA_CA_CERTS=/usr/local/share/ca-certificates/caddymanager-root-ca.crt")
                }
                .font(.caption)
