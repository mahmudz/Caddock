...
struct VhostEditorView: View {
    ...
    private var kindSpecificSection: some View {
        ...
        case .phpSite:
            Section {
                ...
                Text("Serves files from this folder, routing .php requests to PHP-FPM over a unix socket or TCP address.")
            }
        case .reverseProxy:
            Section {
    ...
    private var documentRootBinding: Binding<String> {
        Binding(get: { vhost.documentRoot ?? "" }, set: { vhost.documentRoot = $0.isEmpty ? nil : $0 })
    }

    private var phpSocketPathBinding: Binding<String> {
        Binding(get: { vhost.phpSocketPath ?? "" }, set: { vhost.phpSocketPath = $0.isEmpty ? nil : $0 })
    }

    private var proxyTargetBinding: Binding<String> {
        Binding(get: { vhost.proxyTarget ?? "" }, set: { vhost.proxyTarget = $0.isEmpty ? nil : $0 })
    }
