...
final class LocalDNSResponder {
    ...
    func start() {
        ...
        do {
            ...
            let port = NWEndpoint.Port(rawValue: UInt16(HelperConstants.dnsListenPort))!
    ...
    private func buildResponse(for request: Data) -> Data? {
        ...
        // Only answer A (1) and AAAA (28) — AAAA gets empty success to avoid fallback delays.
        if qtype == 28 {
        ...
        response.append(contentsOf: [0x00, 0x01]) // QDCOUNT
        response.append(contentsOf: [0x00, 0x01]) // ANCOUNT
        response.append(contentsOf: [0x00, 0x00]) // NSCOUNT
        response.append(contentsOf: [0x00, 0x00]) // ARCOUNT
        // Question
    ...
    private func parseName(_ data: Data, offset: Int) -> (String, Int)? {
        ...
        while i < data.count {
            ...
            if length & 0xC0 == 0xC0 {
                // compression pointer — unusual in questions; reject
                return nil
            }
